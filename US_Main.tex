...
\subsubsection{Closeness centrality - Reeplace with eigen vector?}

The closeness centrality measures accounts for how close in term of distance is a node to other nodes in a network.

Let $d_{ij}$ be the geodesic distance (shortest path) between node $i$ and $j$ \citep{marin_review_2022}. The mean geodesic distance $l$ of a node $i$ to other nodes in a network is given by
...
The betweenness centrality measure calculates the number of shortest paths between all possible node pairs in a network that go though a node \citep{arcaute_networks_2023}.
...
\citep{newman_networks_2018} cited by \citep{marin_review_2022}
